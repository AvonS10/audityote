...
public class PostureReportService {
    ...
    public PostureReportService(PostureService postureService, FindingService findingService,
                                ...
                                AuditLogRepository auditLog, UserRepository users, ReportFactory reportFactory) {
    ...
    private ReportDocument compose(PostureResponse p, List<FindingSummary> all,
        ...
        return new ReportDocument(
                ...
                List.of(
                        "Generated " + TS.format(now),
                        "Generated by " + actorLine(actorEmail),
    ...
    private ReportSection.TableSection topRisksTable(List<FindingSummary> all, Instant now) {
        ...
        List<List<String>> rows = top.stream().map(f -> List.of(
                ...
                f.riskScore() == null ? "" : f.riskScore().toPlainString()
                        + ("derived".equals(f.riskSource()) ? " (der)" : ""),
    ...
    private String actorLine(String email) {
        return users.findByEmail(email)
                .map(u -> u.getName() + " (" + capitalize(u.getRole().name().toLowerCase(Locale.ROOT)) + ")")
                .orElse(email);
    }
